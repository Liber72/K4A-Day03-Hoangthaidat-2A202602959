...
class MCPFinanceServer:
    ...
    """
    def __init__(self, server_name: str = "finance-assistant-mcp-server"):
...
if __name__ == "__main__":
    print("==========================================================")
    print("🔌 KIỂM THỬ ĐỘC LẬP MCP SERVER (finance-assistant-mcp-server)")
    print("==========================================================")
    ...
    order_tool = next((t for t in tools if t.get("name") == "place_order"), None)
    if order_tool and not order_tool.get("parameters", {}).get("properties"):
        print("⏳ [TODO 1.2]: Tool 'place_order' chưa được định nghĩa properties trong 'src/tools.py'.")
    else:
        print("✅ [TODO 1.2]: Tool 'place_order' đã có schema đầy đủ.")

    # Kiểm tra trạng thái TODO 2.1 (call_tool)
    test_result = server.call_tool("fetch_stock_info", {"ticker": "FPT"})
    if not test_result:
        print("⏳ [TODO 2.1]: Hàm call_tool() đang trả về rỗng. Học viên hãy hoàn thiện TODO 2.1 trong 'src/mcp_server.py'!")
    else:
        print(f"✅ [TODO 2.1]: Test dispatch tool 'fetch_stock_info' thành công:")
